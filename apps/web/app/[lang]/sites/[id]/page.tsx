...
const Page = async ({ params: { lang = defaultLocale, id } }: SiteProps) => {
  ...
  site.studies.filter((site) => site.status === studyStatus.RECRUITING).map((st) => (
    st.translations.find(trans => trans[lang])[lang].conditions_ct.map((cond) => {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 xl:grid-cols-4 gap-14">
        <div className="w-full col-span-1 lg:col-span-2 xl:col-span-3 pr-0">
          ...
          <div className="h-full flex-col flex gap-4">
            <H3 label={site.name} className="my-0 sm:my-0 lg:my-0 xl:my-0 !pb-0" />
            ...
            <p className='text-dark font-normal font-["DMSans"] leading-relaxed '>{site.description}</p>
          </div>
        </div>

        <div className="col-span-1 lg:row-span-2 pl-0 flex flex-col gap-12 lg:gap-10">
          <div className="flex flex-col justify-start items-start" style={{ backgroundColor: 'green' }}>
            <p>Card Flotante</p>
          </div>
        </div>
        ...
        <div className="w-full col-span-1 lg:col-span-2 xl:col-span-3 pr-0 flex-col justify-start items-start gap-14 inline-flex divide-y divide-gray-medium">
          <div className='w-full flex-col justify-start items-start gap-4 inline-flex'>
            <H4 label={t('specializations')} className="text-primary my-0 lg:my-0" />
            ...
            <div className='w-full pt-14 flex-col justify-start items-start gap-4 inline-flex'>
              ...
              <div className="self-stretch flex-col justify-start items-start gap-4 flex">
                {site.perks?.map((p, index) => (
                  <div key={index} className="self-stretch justify-start items-center gap-2 inline-flex">
                    <CheckIcon color='dark' />
                    <div className="text-dark text-base font-normal font-['DMSans'] leading-none">{p}</div>
                  </div>
                ))}
              </div>
            </div>
          ...
          { site.physicians?.length > 0 && (
            <div className='w-full pt-14 flex-col justify-start items-start gap-4 inline-flex'>
              <H4 label={t('physicians')} className="text-primary my-0 lg:my-0" />
              <div className="w-full grid sm:grid-cols-3 gap-x-36 gap-2">
                {site.physicians.map((py) => (
                  <div key={py.id} className="text-dark text-base font-normal font-['DMSans']">{`${py.first_name} ${py.last_name}`}</div>
                ))}
              </div>
            </div>
          )}
          ...
          {conditionsList.length > 0 && (
            <div className='w-full pt-14 flex-col justify-start items-start gap-4 inline-flex'>
              <H4 label="Estudios Abiertos" className="text-primary my-0 lg:my-0" />
        ...
        {site.site_image && (
          <div
            className="w-full col-span-1 lg:col-span-2 xl:col-span-3 pr-0 flex-col justify-start items-start gap-14 inline-flex mb-14">
